    ...
    public class TinyCarController : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (onGround)
            {
                ...
                velocity += crossForward * motor * accelerationForce * deltaTime;
            }

            velocity += gravityDirection * Mathf.Min(Mathf.Max(0, maxGravity + velocity.y), gravityVelocity * deltaTime);
            if (velocity.sqrMagnitude > maxSpeedSqr) velocity = velocity.normalized * maxSpeed;
        ...
        {
            float accelerationCurveDelta = Mathf.Abs(forwardVelocity) / (maxSpeed * speedMultiplier);
            if (accelerationCurveDelta > 1) return -maxAcceleration;
            return accelerationCurve.Evaluate(accelerationCurveDelta) * maxAcceleration * accelerationMultiplier;
        }
        ...
        {
            return Mathf.Abs(getForwardVelocity()) / maxSpeed;
        }
        ...
        {
            return getGroundVelocity() / maxSpeed;
        }
